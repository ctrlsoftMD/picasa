            ...
            while (i < settings.albumMaxResults && i < $na && i < (settings.albumsPerPage * settings.albumPage)) {
                ...
                if ((($.inArray(j.feed.entry[i].gphoto$name.$t, settings.albums) > -1) || 
                    ...
                    if ($keywordMatch == true) {
                        ...
                        if (settings.showAlbumThumbs) {
                            $scAlbum.append("<div class='thumb_back'><img src='" + $thumb + "'/></div>");
                        }
                        if (settings.showAlbumTitles) {
                            $item_plural = (j.feed.entry[i].gphoto$numphotos.$t == "1") ? false : true;
                            $scAlbum.append("<br/>" + ( (j.feed.entry[i].title.$t.length > settings.showAlbumTitlesLength) ? j.feed.entry[i].title.$t.substring(0, settings.showCaptionLength) : j.feed.entry[i].title.$t) + "<p><small>" + (settings.showAlbumPhotoCount ? "&nbsp;&nbsp;&nbsp;&nbsp;" + j.feed.entry[i].gphoto$numphotos.$t + " " + ($item_plural ? settings.labels.photos :  settings.labels.photo) : "")+"</small></p>");
                        }
...
;
    ...
    $.fn.pwi.defaults = {
        ...
        labels: {
            ...
            albums: "Inapoi la album",
            slideshow: "Display slideshow",
            noalbums: "Nu exista albume",
            loading: "Loading...",
            page: "Pagina",
            prev: "Precedent",
            next: "Urmator",
